...
const PublicExchange = () => {
  ...
  const [exchange, setExchange] = useState<PublicDocumentData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(false);
  const invoiceRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const fetchPublicExchange = async () => {
      try {
        const response = await axios.get(
          `${Constants.BASE_URL}/api/public/exchanges/${publicShareId}`,
        );
        if (response.data.success) {
          setExchange(normalizePortalInvoice(response.data.data));
        } else {
          setError(true);
        }
      } catch {
  ...
  useEffect(() => {
    if (exchange) document.title = `Exchange ${exchange.exchangeNumber || exchange.invoiceNumber} - ${exchange.business.name}`;
  }, [exchange]);

  const handlePrint = useReactToPrint({
    contentRef: invoiceRef,
    documentTitle: exchange?.exchangeNumber || exchange?.invoiceNumber || "Exchange",
    pageStyle: `
      @page {
        size: auto;
    ...
    return (
      <div className="flex min-h-[100dvh] items-center justify-center bg-[#f8f6f3] px-4">
        <div className="w-full max-w-[420px] rounded-2xl border border-stone-200 bg-white p-7 text-center shadow-sm">
          ...
          <h1 className="mb-2 font-serif text-xl font-bold text-stone-950">Exchange Not Found</h1>
          <p className="text-sm leading-6 text-stone-600">
            This exchange link is invalid, expired, or no longer active.
          </p>
        </div>
      </div>
    );
  ...
  const invoiceTemplateData = toInvoiceTemplateData(exchange);

  return (
    <div className="min-h-[100dvh] overflow-x-hidden bg-[#eeeae5]">
      <PublicDocumentPortal documentType="EXCHANGE" documentData={exchange} onDownload={() => handlePrint()} />
      ...
      >
        <InvoiceTemplateB
          ...
          companyDetails={{
            name: exchange.business.name,
            address: exchange.business.address,
            phone: exchange.business.phone,
            logo: exchange.business.logo,
