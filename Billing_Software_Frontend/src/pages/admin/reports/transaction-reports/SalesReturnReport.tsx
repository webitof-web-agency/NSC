    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-gray-600"> Sales Return Report </h1>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatsCard
                    title="Total Returned Amount"
                    value={format(chartData?.TotalRefund.currentMonthAmount || 0)}
                    difference={totalRefundPercentage || 0}
                    icon={<BiMoneyWithdraw size={25} className={getColorClass(totalRefundPercentage)} />}
                    color={getColorClass(totalRefundPercentage, true)}
                />
                <StatsCard
                    title="Active Invoices Exchanged"
                    value={chartData?.ActiveCreditNotes.currentMonthCount || 0}
                    difference={activeCreditNotesPercentage || 0}
                    icon={<CheckCircleIcon size={25} className={getColorClass(activeCreditNotesPercentage)} />}
                    color={getColorClass(activeCreditNotesPercentage, true)}
                />
                <StatsCard
                    title={`Most Returned Product` + (chartData?.MostReturnedProduct.name
                        ? ` - ${chartData?.MostReturnedProduct.name}${chartData?.MostReturnedProduct.variantName ? ` (${chartData?.MostReturnedProduct.variantName})` : ''}`
                        : '')}
                    value={chartData?.MostReturnedProduct.currentMonthQty || 0}
                    difference={mostReturnedProductPercentage || 0}
                    icon={<Box size={25} className={getColorClass(mostReturnedProductPercentage || 0)} />}
                    color={getColorClass(mostReturnedProductPercentage, true)}
                />
            </div>
            {/* Filters*/}
            ...
            {/* Table */}
            <Table headers={['#', 'Invoice ID', 'Customer', 'Total Amount', 'Payment Mode', 'Status', 'Credit Note Date']}>
                ...
                {!isLoading && salesReturnReport.length === 0 && (
                    <tr>
                        <td colSpan={7} className="text-center py-4 text-gray-600 font-semibold">No records found</td>
                    </tr>
                )}
